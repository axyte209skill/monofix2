...
import { ACTION_KEYS, CARD_TYPES } from "./models/Card.js";
...
/** Applies data-en/data-hi translations to every matching element inside `root` only. */
...
function applyLanguage(lang) {
  ...
  // Re-render dynamic (non data-en/hi) strings immediately. IMPORTANT: renderGame()
  // must never call applyLanguage() itself (directly or indirectly) — doing so
  // previously created infinite mutual recursion between the two functions,
  // which crashed every in-game render before it could reach the turn status,
  // the deck/discard piles, or the property/bank boards. renderGame() now
  // calls translateSubtree() on just the nodes it builds instead.
...
socket.on("disconnect", () => {
  ...
  connectionBannerEl.textContent = t("disconnectedReconnecting");
});

socket.on("connect_error", (err) => {
  console.error("[Monopoly Deal] connection error:", err);
  connectionBannerEl.hidden = false;
  connectionBannerEl.textContent = t("connectionFailed");
});
...
function renderGame() {
  ...
  latestGame.opponents.forEach((opp) => {
    ...
    nameRow.innerHTML = `<span class="turn-dot"></span> <span class="opponent__name-text">${opp.name}${status}</span><span class="opponent__hand-count">${opp.handCount}🂠</span>`;
  ...
  // Translate the data-en/data-hi labels just built above (e.g. "Properties" /
  // "Bank"), without calling applyLanguage() — see the comment on applyLanguage().
  translateSubtree(opponentsEl);

  renderDeckPile(deckPileEl, latestGame.deckCount);
  renderDiscardPile(discardPileEl, latestGame.discardTop, latestGame.discardCount);
  ...
  gameStatusEl.classList.toggle("game-status--mine", isMyTurn && !latestGame.winner);
  myAreaEl?.classList.toggle("my-area--turn", isMyTurn && !latestGame.winner);
...
playCardBtn.addEventListener("click", () => {
  ...
  if (card.type === CARD_TYPES.WILD_PROPERTY && Array.isArray(card.color) && card.color.length > 1) {
    renderTargetPicker(
      targetPickerEl,
      t("wildColorPrompt"),
      card.color.map((color) => ({
        label: color,
        onPick: () => commitPlayCard(card.id, { colorChoice: color }),
      }))
    );
    return;
  }
