...
/**
 * Builds one face-up card element from a Card instance, colour-coded by
 * card type so cards are recognisable at a glance:
 *  - Property cards: a solid colour band (their set colour) over a light body.
 *  - Wild property / two-colour Rent cards: a diagonal split of both colours.
 *  - Wild ("any colour") Rent cards: a rainbow gradient.
 *  - Money cards: a green "cash" card showing the denomination.
 *  - Plain action cards: a violet card showing the action name.
 */
function buildCardElement(card) {
  ...
  const isRent = card.actionKey === ACTION_KEYS.RENT;

  if (card.type === CARD_TYPES.PROPERTY) {
    el.classList.add("card--property");
    el.style.setProperty("--card-color", colorToCss(card.color));
  } else if (card.type === CARD_TYPES.WILD_PROPERTY && Array.isArray(card.color)) {
    el.classList.add("card--split");
    el.style.setProperty("--card-color-1", colorToCss(card.color[0]));
    el.style.setProperty("--card-color-2", colorToCss(card.color[1] ?? card.color[0]));
  } else if (isRent && card.color === "any") {
    el.classList.add("card--rainbow");
  } else if (isRent && Array.isArray(card.color)) {
    el.classList.add("card--split");
    el.style.setProperty("--card-color-1", colorToCss(card.color[0]));
    el.style.setProperty("--card-color-2", colorToCss(card.color[1] ?? card.color[0]));
  ...
  if (card.type === CARD_TYPES.MONEY) {
    ...
    value.textContent = `$${card.moneyValue}M`;
    ...
    if (card.type === CARD_TYPES.PROPERTY && Array.isArray(card.rentValues)) {
      const rent = document.createElement("div");
      rent.className = "card__rent";
      rent.textContent = `Rent ${card.rentValues.join("/")}`;
...
/** Renders the draw pile: a face-down card back plus how many cards remain. */
export function renderDeckPile(containerEl, deckCount) {
  if (!containerEl) return;
  let cardEl = containerEl.querySelector(".card");
  if (!cardEl) {
  ...
  let countEl = containerEl.querySelector(".pile__count");
  if (!countEl) {
    countEl = document.createElement("div");
    countEl.className = "pile__count";
    containerEl.appendChild(countEl);
  }
  countEl.textContent = `${deckCount ?? 0} left`;
}

/** Renders the discard pile: the real face-up top card (if any) plus a count. */
export function renderDiscardPile(containerEl, topCard, discardCount) {
  if (!containerEl) return;
  const oldCard = containerEl.querySelector(".card");
  if (oldCard) oldCard.remove();
  const oldCount = containerEl.querySelector(".pile__count");
  if (oldCount) oldCount.remove();

  const cardEl = topCard ? buildCardElement(topCard) : document.createElement("div");
  if (!topCard) cardEl.className = "card card--empty";
  containerEl.appendChild(cardEl);

  const countEl = document.createElement("div");
  countEl.className = "pile__count";
  countEl.textContent = `${discardCount ?? 0}`;
  containerEl.appendChild(countEl);
}
