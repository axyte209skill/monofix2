...
export class GameEngine {
  ...
  playCard(playerId, cardId, options = {}) {
    ...
    if (card.type === CARD_TYPES.PROPERTY || card.type === CARD_TYPES.WILD_PROPERTY) {
      // The client sends the chosen colour nested under `target` (same shape
      // as every other card option), so check there first; `colorChoice` at
      // the top level is kept as a fallback for direct/engine-level callers.
      const colorChoice = options.target?.colorChoice ?? options.colorChoice ?? null;
      board.addPropertyCard(card, colorChoice);
